...
export default function App() {
  ...
  const formatNumberText = (value) => {
    ...
    const formattedInteger = new Intl.NumberFormat("ja-JP").format(
  ...
  const data = useMemo(() => {
    ...
    return Array.from({ length: months }, (_, i) => {
      ...
      const pv =
        toNumber(monthlyPv) +
        toNumber(monthlyPvIncrease) * month;

      const inquiriesRaw =
        pv * (toNumber(inquiryRate) / 100);

      const inquiries = roundContracts
        ? Math.round(inquiriesRaw)
        : Number(inquiriesRaw.toFixed(1));

      const newContractsRaw =
        inquiriesRaw * (toNumber(closeRate) / 100);
      ...
      const monthlyRevenue =
        contractBaseForRevenue *
        toNumber(monthlyRevenuePerContract);
      ...
      const cumulativeProfit =
        cumulativeRevenue - cumulativeCost;
  ...
  const paybackMonth =
    data.find((item) => item.cumulativeProfit >= 0)?.month ||
    "未回収";

  const roi =
    last && last.cumulativeCost !== 0
      ? ((last.cumulativeRevenue - last.cumulativeCost) /
          last.cumulativeCost) *
        100
      : 0;
    ...
    <main style={styles.page}>
      ...
      <div style={styles.container}>
        ...
        <section style={styles.summaryGrid}>
          <Summary
            title="累計利益"
            value={yen(last?.cumulativeProfit)}
            negative={last?.cumulativeProfit < 0}
          />
          ...
          <Summary
            title="累計契約数"
            value={
              displayContractCount(last?.activeContracts) + "件"
            }
          />
        </section>
        ...
        <section style={styles.layout} className="main-layout">
          <div style={styles.card}>
            ...
            <Field
              label="毎月のPV増加見込み"
              value={monthlyPvIncrease}
              onChange={setMonthlyPvIncrease}
              suffix="PV"
              sanitizeNumber={sanitizeNumber}
              formatNumberText={formatNumberText}
            />
            ...
            <Field
              label="1契約あたり月額売上"
              value={monthlyRevenuePerContract}
              onChange={setMonthlyRevenuePerContract}
              suffix="円"
              sanitizeNumber={sanitizeNumber}
              formatNumberText={formatNumberText}
            />
            ...
            <Field
              label="月額運用・記事費用"
              value={monthlyContentCost}
              onChange={setMonthlyContentCost}
              suffix="円"
              sanitizeNumber={sanitizeNumber}
              formatNumberText={formatNumberText}
            />
            ...
            <label style={styles.checkboxRow}>
              <input
                ...
                onChange={(e) =>
                  setRoundContracts(e.target.checked)
                }
              />
          ...
          <div style={styles.card}>
            ...
            <p style={styles.resultLead}>
              ...
              <strong
                style={
                  last?.cumulativeProfit < 0
                    ? styles.negativeText
                    : undefined
                }
              >
                {yen(last?.cumulativeProfit)}
              </strong>
              、ROIは{" "}
              <strong
                style={
              ...
              、回収月は <strong>{paybackMonth}</strong> です。
            </p>
                ...
                <tbody>
                  {data.map((item) => (
                    <tr key={item.month}>
                      ...
                      <Td>
                        {displayContractCount(item.inquiries)}
                      </Td>

                      <Td>
                        {displayContractCount(item.newContracts)}
                      </Td>

                      <Td>
                        {displayContractCount(item.activeContracts)}
                      </Td>
